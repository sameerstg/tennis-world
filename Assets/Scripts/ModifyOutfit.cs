...
public class ModifyOutfit : MonoBehaviour
{
    ...
    public void SetOutfit(bool editor)
    {
        if (outfit == null)
            return;
        if (hatRenderers == null || hatRenderers.Length != hats.Length)
        {
            hatRenderers = new Renderer[hats.Length];
            for (int i = 0; i < hats.Length; i++)
            {
                hatRenderers[i] = hats[i].GetComponent<Renderer>();
            }
        }
